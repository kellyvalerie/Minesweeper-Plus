        ...
        if start_limited_digs == True:
            ...
            for row in range(dimension):
                for column in range(dimension):
                    if [row, column] in checked:
                        continue
                    closed_cells.append([row, column]) #append to unchhecked (list of unrevealed cells) 
            remaining_digs = len(closed_cells) - 2 - len(revealed_bomb_location) - extra_limited_digs #total remaining digs for the user = number closed cells + number of revealed bombs -2(from initialization)
...
""")
            ...
            if board[row][column] != '💣':
                if board[row][column] > 0:
                    if random.random() < 1.00:  #50% chance
                        ...
                        if takepowerup.lower() == 'yes' or takepowerup.lower() == 'y':
                            powerup_list = ['reveal 1 bomb', 'second life', 'add time', 'bompsweep', 'immunity', 'limited digs', 'close 1 cell', 'reduce time']
                            temp = [0, 0, 0, 0, 0, 0, 100, 0]
                            weights = [10, 15, 20, 5, 15, 10, 10, 15]
                            ...
                            if chosen_powerup[0] == 'reveal 1 bomb':
                                ...
                                time.sleep(2)
                                safe_dig(board)
                            elif chosen_powerup[0] == 'limited digs':
