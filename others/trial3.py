...
- Hard (22x22)""")
    ...
    while len(checked) < (dimension**2 - numbombs):
        ...
        mode = input('''Modes: Dig (D), Flag (F), Unflag (UF), Questionmark (Q), Unquestionmark (UQ), Bomb Sweep (BS)
Choose (D, F, UF, Q, UQ, BS): ''')
        ...
        elif mode.lower() in ['bs', 'bomb sweep']:
            if bomb_sweep_available:
                bomb_sweep(board, dimension)
                bomb_sweep_available = False
            else:
                print("You've already used your Bomb Sweep!")
