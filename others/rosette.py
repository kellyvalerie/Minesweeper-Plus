...
def validate_input(name):
    while True:
        ...
        if len(cell_input) >= 2 and cell_input[0].isalpha() and cell_input[1:].isdigit():
            if cell_input[0].upper() in row_description and 1 <= int(cell_input[1:]) <= dimension:
...
def randomreveal(board):
    ...
    revealed = random.choice(unchecked)#randomly choose a unrevealed cell
    #if the chosen cell is a bomb, the user loses
    safe = mining(board, revealed[0], revealed[1])
    if safe == False:
        print("""
...
""")    
        ...
        print(f'Revealed cell: {chr(ord('A') + revealed[0])}{revealed[1]+1} 🔍✨')
        time.sleep(2)
...
''')     
    ...
    while len(checked) < (dimension**2 - numbombs - len(revealed_bomb_location)):
        ...
        if start_limited_input == True:
            input_total = (numbombs-correct_bombs)*2 #total remaining inputs for the user = number remaining bombs * 2
                ...
                if board[row][column] > 0:
                    if random.random() < 0.50:  #50% chance
                        ...
                        if takepowerup.lower() == 'yes' or takepowerup.lower() == 'y':
                            powerup_list = ['reveal 1 bomb', 'second life', 'add time', 'bompsweep', 'immunity', 'limited input', 'random reveal', 'reduce time']
                            weights = [0, 0, 0, 0, 0, 100, 0, 0]
                            chosen_powerup = random.choices(powerup_list, weights=weights, k = 1) #randomly choose powerups, with different probabilities(weight)
                            ...
                            elif chosen_powerup[0] == 'random reveal':
                                print("I will randomly dig a cell for you, maybe it's a bomb 😈😈")
                                time.sleep(2)
                                safe_random_reveal = randomreveal(board)
                                if safe_random_reveal == False:
                                    return 0
